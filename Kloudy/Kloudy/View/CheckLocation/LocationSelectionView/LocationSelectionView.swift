...
class LocationSelectionView: UIViewController {
    ...
    // 해당 시점에는 view 가 없는 초기화 코드, view 와 관계 없는 object 초기화 용도로 굳
    ...
    private func layout() {
        [locationSelectionNavigationView, searchBar, cancelSearchButton].forEach { view.addSubview($0) }
        ...
        searchBar.snp.makeConstraints {
            $0.leading.trailing.equalToSuperview().inset(12)
            $0.top.equalTo(locationSelectionNavigationView.snp.bottom).offset(20)
            $0.height.equalTo(47)
        }
    ...
    private func configureCancelSearchButton() {
        ...
        cancelSearchButton.titleLabel?.sizeToFit()
        cancelSearchButton.titleLabel?.font = UIFont.KFont.appleSDNeoRegularLarge
        cancelSearchButton.addTarget(self, action: #selector(endEditing), for: .touchUpInside)
    }
}
